...
export function VisiMisi() {
  ...
  return (
    <section id="profil" className="relative overflow-hidden bg-hero-gradient py-20 sm:py-28">
      {/* Foto gedung kampus sebagai latar, tanpa lapisan penggelapan — tampil
          bersih apa adanya. Teks di atasnya tetap terbaca lewat text-shadow
          (visi) dan panel kaca (misi), bukan lewat menutupi fotonya. */}
      <img src={kampusIvet} alt="" aria-hidden loading="lazy" className="absolute inset-0 h-full w-full object-cover" />
            ...
            <ol className="mt-5">
              {misi.items.map((item, i) => (
                ...
                >
                  ...
                  <span className="text-sm leading-relaxed text-hero-foreground/90 xl:text-base">{item}</span>
                </li>
              ))}
            </ol>
          </div>

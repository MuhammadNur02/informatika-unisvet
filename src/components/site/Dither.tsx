...
// Komponen "Dither" dari React Bits (reactbits.dev/backgrounds/dither) — latar
// noise bergelombang yang di-dither jadi beberapa warna diskrit (gaya
// retro/CRT), bereaksi ke posisi mouse. Diadaptasi ke TypeScript dari sumber
// JSX resminya; shader & logiknya dipertahankan apa adanya, hanya ditambah
// anotasi tipe (beberapa lewat cast karena tipe publik
// @react-three/postprocessing tidak mencakup custom setter property seperti
// `colorNum`/`pixelSize` di bawah — itu properti ad-hoc, bukan bagian
// constructor efeknya).
...
class RetroEffectImpl extends Effect {
  constructor() {
    ...
    // `uniforms` sudah tersimpan sebagai `this.uniforms` oleh super() (readonly
    // di tipe publiknya) — beda dari sumber JSX resmi yang menimpanya lagi
    // secara manual sesudah ini (no-op yang tidak lolos TypeScript).
...
const RetroEffect = forwardRef<RetroEffectImpl, RetroEffectProps>(
  ({ colorNum, pixelSize }, ref) => {
    // wrapEffect() menurunkan tipe props dari parameter constructor RetroEffectImpl
    // (kosong, di atas) — colorNum/pixelSize bukan argumen constructor, tapi
    // property ad-hoc yang di-assign wrapEffect ke instance efeknya saat runtime.
    // Tipe publiknya tidak mencakup ini, jadi di-cast manual di sini.
    const Wrapped = WrappedRetro as unknown as ComponentType<
      RetroEffectProps & { ref?: Ref<RetroEffectImpl> }
    >;
